...
**产品形态与功能范围照 WorkBuddy（它是规格书），底层机制与工程纪律在同类实现里择优。**
逆向调研素材在 `docs/workbuddy分析/`；同类实现的对照记录见 `docs/dsh-对照记录.md`
与 `docs/多agent参考实现横向对照.md`。
...
参照物是**并列**的，**没有「先模仿 WorkBuddy」这个默认**：

- **WorkBuddy 是产品规格书**——决定「做什么」：功能清单、交互形态、办公场景的边界。
- **codex / dsh / opencode / Trae / pi 是同类实现的设计源**——决定「怎么做更好」：
  底层机制、工程纪律、门禁与证据体系。

两者是**正交的两根轴，不是主备关系**。动手前按下面的顺序决定怎么做：

1. **市面已有成熟实现 → 直接用。** 成熟库能覆盖的（文档解析、格式转换、图表等），
   不自己写，避免堆出不可维护的私货。可扩展、可维护优先于「自己造」。
2. **同类 Agent 实现横向对照 → 择优。** 至少比 WorkBuddy / dsh / codex 三家；
   产品形态与底层机制分开评，**机制哪家强用哪家**。
   检索入口：`开源项目/{WorkBuddy,pi,opencode,deepseek-harness}/`、
   `docs/workbuddy分析/`、`docs/dsh-对照记录.md`、`docs/多agent参考实现横向对照.md`。
   对照不是浏览而是**留痕**：写清各家的做法与出处（文件 + 行号），再写选谁、
   为什么不选另外几家——后者就是决策记录的 `## 否决方案`。
3. **pi 自身的插件 / 扩展生态也要看**（`开源项目/pi/` 的 packages 与 examples），
   不少能力 pi 已有现成参考或可直接复用。
4. **还是定不下来 → 来问我。** 拿不定主意先问，不要擅自选一条路硬写。

上面这条原则不变；它凌驾于「我先按文档写的做」之上——文档写得和它冲突时，以它为优先并回来确认。

这不是新立的方向，是把**既有做法**写进规则（理由与否决方案见 `docs/ARCHITECTURE.md` §4.11）：
Windows 沙箱选了 **dsh 的机制并否决 WorkBuddy 的做法**（§4.4b）；权限词汇**对齐 dsh 与 codex**
（§4.4a、§4.57）；`adopt-dsh-disciplines` 整份 spec 采纳的是 **dsh 的工程纪律**，与 WorkBuddy 无关。
...
- **文档流水线按 WorkBuddy 的方式跑 Python，不再禁止 bash/python。**
  ...
  beautifulsoup4 / lxml / Pillow），环境用**按需安装的托管运行时**解决：
  `python` / `node` / `gitbash` 三个运行时统一放在托管根
  `<configDir>/runtimes/<id>/<version>/` + `current` 指针（内核在 `src/core/runtimes/`），
  **用户什么都不用预装**（不要求预装 Python / Git for Windows）。
  **安装包不含任何运行时载荷**：三个运行时都由用户在「设置 → 内置运行时」按需安装 ——
  点了「安装」才联网下载（python 由 `uv` 装，`--only-binary=:all:`；node / gitbash 由
  `src/core/runtimes/{artifact,download}.ts` 下载 + 校验 sha256 + 解包）。
  **不会静默自动下载**（会话启动预热与首次使用的隐式安装都已撤），**首次使用需联网**；
  离线/内网因此默认不可用 —— 这是被明确接受的代价，须靠 `KAMIBUDDY_NODE_URL` /
  `KAMIBUDDY_GITBASH_URL` / `UV_INDEX_URL` 等覆盖口指向内网镜像才能用（私有化镜像本轮未拍板）。
  来源、校验、许可义务与体积实测见 `docs/运行时来源与许可.md` 与
  `resources/runtimes/README.md`（后者含 §6 的**否决方案**）。
- **环境准备照 WorkBuddy 的** **`setup-html-to-docx.sh`** **抄机制**：
  - 幂等，已就绪秒退；用户点「安装」后联网：装 `uv` → `uv python install 3.12` 拉独立发行版
  ...
  - 私有化/无外网：`UV_INDEX_URL` + `UV_PYTHON_INSTALL_MIRROR` 指向内网镜像，
    或由 `KAMIBUDDY_NODE_URL` / `KAMIBUDDY_GITBASH_URL` 指向内网镜像；
    **不预置镜像/离线产物就装不上任何运行时**（安装包不含载荷，见上）。
  - **不做会话启动预热、不做首次使用的隐式安装**（2026-09-18 起）：那类动作发生在
    用户没点任何按钮的时刻，等于静默自动下载，已被明确否掉。
...
- **agent 的 shell 能力另有决策**（2026-09-17 修订：原「**`bash` 仍然不用**」**作废**）：
  Git Bash 现在**按需安装**作为托管运行时提供（用户不必自己装 Git for Windows —— 但要在
  「设置 → 内置运行时」点安装 `gitbash`，原约束的**前提已消失**），
  ...
  安装了运行时只让它在**路径上找得到**（注入层：`src/core/runtime-inventory.ts` 的
