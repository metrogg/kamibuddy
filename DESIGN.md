...
| `--bg-chip` | rgba(0,0,0,.06) | 内容 chip 的**静态**底色（技能胶囊，§3.6） |
| `--border` | #e6e6e6 | **唯一**常规边框 |

> `--bg-chip` 是 2026-09-16 按用户反馈新增的一档（原本技能胶囊借用 `.context-chip` 的
> 32px 控件视觉，在用户气泡里比正文行高一档）。它与 `--bg-hover` 同族但语义不同：
> hover 是交互反馈、chip 是静态承载；取值与 WB 两个原值（白底 #f2f2f2 / 气泡内
> rgba(0,0,0,.10)）的对齐过程见 tokens.css 的注释。
...
输入卡的「已选技能条」与用户消息气泡里的只读胶囊共用同一个类，视觉对齐 WB 的 `skillTag`
（`lib-chat-ui-Co_VI_pZ.css:12261-12298`）：

| 项 | 规格 |
|---|---|
| 形 | `inline-flex`、高 **23px**、`--radius-full` 胶囊、padding 1px `--space-3`、gap `--space-1` |
| 字 | `--text-list`（13px），不加粗（WB 写 500，本站只有 400/600 两档语义，取常规） |
| 底 | `--bg-chip`（§2.2） |
| 图标 | `IconSkill` 14px（WB skillTag 的图标容器就是 14，比基类 16 小一档） |
| 删除钮 | 复用 `.document-ref-remove` 的声明（16px 圆钮、`--text-secondary`、hover 才显形）；**保留独立按钮**语义（aria-label + 键盘可达），不照搬 WB 的「hover 时与原位图标互换」 |
...
7. 弹层入场只做一次；**条件挂载**的弹层（`{open && <div>}`）不做退场动画（卸载即消失）——
   ...
   入场动画按**元素形态**挑 keyframes：浮层用 `pop-layer-in`（4px 上移 + `scale(.98)`，
   配规则 9 的 `transform-origin`）；**流内展开体**（思考块正文、定时任务运行记录、
   团队状态栏与成员列表、任务诊断折叠体、审批详情）用 `fold-in`（-4px 上移 + 淡入）——
   全宽块上套 `scale(.98)` 会让整块文字跟着缩，读作「抖一下」而不是「展开」。
...
③ **`.preview-panel` 的 `width` 过渡**：全屏切换需要（WB 同参数）；拖拽期间由
   ...
   2026-09-19 **扩到面板开合**：宽度过渡落在常驻容器 `.panel-slot` 上（0 ↔ panelWidth），
   面板本体仍是条件挂载的内容 —— 与例外 ④（侧栏折叠）同一语义：右侧面板也是**让位栏**
   （真的占布局宽度、把消息列推开），`transform` 表达不了，所以动 `width`。
   附则：容器 `overflow: hidden` 负责滑动期间裁掉溢出；关闭时内容延迟 `--dur-base` 再卸载。
   **2026-09-19 订正（全屏动效）**：全屏就是这条 width 从 panelWidth 扫到 `100%`
   —— 全屏的面板 absolute 覆盖**整个窗口（含左侧栏）**，右缘锚定 `right: 0`，
   左缘随 width 向左铺开（内容跟着逐帧重排，同「12 帧布局」代价）。
   为此取消了三处旧写法，它们合起来正是「全屏没动效」的根因：`.fullscreen` 不再
   `transition: none`、不再 `left: 0`（与 `right` 同时给会过度约束、`right` 被丢掉，
   左缘钉死在窗口左边），容器也不再于全屏时把宽度归零（对话列保持停靠态，
   否则在面板只盖住右缘 440px 的第一帧就瞬跳成全宽、再被扫过来的面板盖住）。
   退出方向由 `artifact-panel` 的**视觉相位**多留一档再摘类（摘早了面板已回到在流的
   440px 盒子，那一帧即终态，没有起点可跑）。进出同档 `--dur-base`：全屏是同一个盒子的
   状态切换（面板不离开屏幕），不套 §5.8 的进出场不对称 —— 与例外 ④（侧栏折叠）同款。
...
- [ ] 图标来自 `icons.tsx`（**例外：品牌位**用应用 logo 光栅图 —— 侧栏底部徽标与回合头
      agent 头像共用 `src/renderer/assets/buddy.png`，唯一入口 `src/renderer/buddy-logo.ts`；
      光栅件带不了形状/颜色，故不适用「图标都走 icons.tsx」的收敛目标）；
      没有新 spinner/空态/chip/列表行类名
...
- **技能胶囊的 23px 高 / padding 1px**：WB `skillTag` 原值（§3.6），不是间距档位值。
  23px 是为了贴齐用户气泡的正文行（15px/1.6 = 24px），归到 24 或 28 的档上都会顶出一行。
