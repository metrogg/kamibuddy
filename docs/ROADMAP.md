...
工具目录里搜不到任何越界检查。模型给出绝对路径就能读写硬盘任意位置，
包括 `~/.kamibuddy/auth.json` 与 `~/.ssh` 里的密钥。
pi README 自述 "does not include a built-in permission system" —— 它的方案是靠容器隔离整个进程，我们不能那样做。

**凭据是禁读也禁写的**（2026-09-08 修正）。此前只禁写，理由写的是
"读到也带不走（没有网络工具）" —— **T3 落地 web_fetch 之后这个前提就不成立了**：
提示注入可以诱导「读 auth.json → 抓取某个 URL 把内容带上」。
**任何新增的外发能力（上传、发邮件、调第三方 API）都要重新审一遍这个推理**，
它是一整类漏洞的源头，不只是这一次。
...
| 闸 | 位置 | 管什么 |
|---|---|---|
| 项目信任 | `extensions/project-trust.ts` | **工具层之前**：`.pi/extensions` 是 TS 模块，加载即以本进程权限执行任意代码，权限门拦不到（那不是工具调用） |
| 权限门 | `extensions/permission-policy.ts` | 工具调用的路径归属、沙箱模式、审批策略 |
| 工具自身 | 各工具实现 | 领域内的校验（如 web-fetch 的 SSRF 拦截） |

新增任何**能写文件、执行命令或对外发送数据**的工具时：

1. 在 `permission-policy.ts` 登记（否则落到「未登记 → 询问」兜底分支，
   虽然安全但会让用户被弹窗淹没）；
2. 想清楚它在三档沙箱模式下各自该怎样 —— 尤其 `read-only` 是否必须拒；
3. 若它能把数据发出去，回到上面那条推理重新评估凭据保护面。
