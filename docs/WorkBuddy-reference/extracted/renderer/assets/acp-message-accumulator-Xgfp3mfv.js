...
import { LG as init_common, RG as isInteractiveQuestionTool, Ui as init_libesm, Wi as diffLines, _m as init_chat_types, cm as resolveAcpMetaTimestamp, hm as MessageType, iR as init_http_logger, lR as writeRendererLog, sm as init_acp_timestamp } from "./ui-docs-viewer-C2jT2eXi.js";
...
/**
* 读取 agent 通过 `_meta['codebuddy.ai/rawResponse']` 透出的结构化 WebSearch 结果。
*
* markdown 反解析（parseWebSearchMarkdown）只能还原 title/url/snippet，
* site/favicon 这类字段在 markdown 里无处安放，只能走这条通道。历史会话由
* persisted-transcript-projector 从 providerData.toolResult.rawResponse 重建
* 同名 _meta，因此 replay 也能命中。
*
* 两种键形态都要认：agent-cli 直接写扁平键；agent-client-protocol 的
* AcpConverter.withMeta 会把 _meta 整包重建为嵌套的 `codebuddy.ai` 对象。
*/
function extractStructuredWebSearchResult(update) {
	const meta = update._meta;
	if (!meta || typeof meta !== "object") return;
	const nested = meta["codebuddy.ai"];
	const rawResponse = (nested && typeof nested === "object" ? nested.rawResponse : void 0) ?? meta["codebuddy.ai/rawResponse"];
	if (!rawResponse || typeof rawResponse !== "object") return;
	const payload = rawResponse;
	if (payload.type !== "web_search_tool_result") return;
	return {
		type: "web_search_tool_result",
		query: typeof payload.query === "string" ? payload.query : void 0,
		searchType: "text2text",
		provider: typeof payload.provider === "string" ? payload.provider : void 0,
		results: Array.isArray(payload.results) ? payload.results : [],
		images: Array.isArray(payload.images) ? payload.images : [],
		totalResults: typeof payload.totalResults === "number" ? payload.totalResults : void 0,
		responseTimeMs: typeof payload.responseTimeMs === "number" ? payload.responseTimeMs : void 0
	};
}
/**
...
function convertToWebSearchResult(update, existingResult, fallbackArgs) {
	...
	const structured = extractStructuredWebSearchResult(update);
	if (structured) {
		const results = structured.results.length > 0 ? structured.results : existingResult?.results || [];
		const images = structured.images.length > 0 ? structured.images : existingResult?.images || [];
		return {
			type: "web_search_tool_result",
			query: argsQuery || existingQuery || structured.query || "",
			searchType: "text2text",
			provider: structured.provider || existingResult?.provider,
			results,
			images,
			totalResults: structured.totalResults || results.length || existingResult?.totalResults,
			responseTimeMs: structured.responseTimeMs || existingResult?.responseTimeMs
		};
	}
	let textContent = "";
...
var init_acp_message_accumulator = __esmMin((() => {
	...
	ACPMessageAccumulator = class ACPMessageAccumulator {
		/**
		* 非终态工具状态（pending tool 判定用，`listPendingToolCallIds` 与
		* `markPendingToolsAsFailed` 共用，改动需两处同步）。
		*
		* 语义：工具尚未给出最终结果。命中其中任一状态都视为"仍在执行中"。
		*/
		static PENDING_TOOL_STATUSES = [
			"idle",
			"pending",
			"parsing",
			"stream_executing",
		...
		*/
		getOrCreateState(sessionId, notification) {
			...
			if (!state || state.pendingReset) {
				...
				if (!state) {
					state = {
						...
						parentTextIndex: /* @__PURE__ */ new Map()
					};
			...
			state.lfConvId = this.extractLangfuseMeta(notification, "lfConvId") || state.lfConvId;
			state.lfConvReqId = this.extractLangfuseMeta(notification, "lfConvReqId") || state.lfConvReqId;
		...
		*/
		hydrateFromAssistantMessage(sessionId, message) {
			...
			const state = existing ?? {
				...
				...message.extra?.lfConvId ? { lfConvId: message.extra.lfConvId } : {},
				...message.extra?.lfConvReqId ? { lfConvReqId: message.extra.lfConvReqId } : {},
			...
			if (!state.lfConvId && message.extra?.lfConvId) state.lfConvId = message.extra.lfConvId;
			if (!state.lfConvReqId && message.extra?.lfConvReqId) state.lfConvReqId = message.extra.lfConvReqId;
			...
			state.parentTextIndex = /* @__PURE__ */ new Map();
		...
		*/
		...
		/** 提取 Langfuse 元数据。 */
		extractLangfuseMeta(notification, key) {
			const metaNotification = notification;
			const metaSources = [metaNotification._meta, metaNotification.update?._meta];
			for (const meta of metaSources) {
				if (!meta || typeof meta !== "object") continue;
				const codebuddyMeta = meta["codebuddy.ai"];
				const nestedValue = codebuddyMeta && typeof codebuddyMeta === "object" ? codebuddyMeta[key] : void 0;
				if (typeof nestedValue === "string" && nestedValue) return nestedValue;
				const flatValue = meta[`codebuddy.ai/${key}`];
				if (typeof flatValue === "string" && flatValue) return flatValue;
			}
		}
		/**
		* 从 _meta 中提取 modelId
		...
		*
		* #87110: 键名必须同时认 parentToolCallId 与 parentToolUseId 两种拼写。
		* agent-cli 本地链路（acp-utils）发的是扁平 `codebuddy.ai/parentToolCallId`，
		* 而云端 teams 的 notification 走的是嵌套 `_meta['codebuddy.ai'].parentToolUseId`
		* （实测生产日志：一个 3 sub-Agent 会话里 parentToolCallId 出现 0 次、
		* parentToolUseId 1997 次）。只认前者会让云端所有 chunk 的 parent 都解析成
		* undefined，#68586 的分桶从未生效，多个 sub-Agent 退化成同一个 writer。
		*/
		extractParentToolUseId(notification) {
			const metaSources = [notification._meta, notification.update?._meta];
			for (const meta of metaSources) {
				if (!meta || typeof meta !== "object") continue;
				const cbMeta = meta["codebuddy.ai"] && typeof meta["codebuddy.ai"] === "object" ? meta["codebuddy.ai"] : void 0;
				const candidates = [
					cbMeta?.parentToolCallId,
					cbMeta?.parentToolUseId,
					meta["codebuddy.ai/parentToolCallId"],
					meta["codebuddy.ai/parentToolUseId"]
				];
		...
		*   若 chunk 属于某个 sub-Agent，则使用该 parent 独立维护的 lastReasoningIndex/
		*   lastTextIndex，避免不同 sub-Agent（或 sub-Agent 与主 Agent）的文本被错误地
		*   追加到同一个 contentBlock（叠印），或反过来互相抢占「数组尾部」资格导致
		*   每个 chunk 都新建一个块（碎片化，"深度思考"被切成零碎段落）。
		*/
		handleTextChunk(state, update, type, parentToolUseId) {
			...
			if (!text) return;
			const useParentIndexing = Boolean(parentToolUseId);
			const parentIndexMap = type === "text" ? state.parentTextIndex : state.parentReasoningIndex;
			const otherParentIndexMap = type === "text" ? state.parentReasoningIndex : state.parentTextIndex;
			const globalIndexKey = type === "text" ? "lastTextIndex" : "lastReasoningIndex";
			const otherGlobalIndexKey = type === "text" ? "lastReasoningIndex" : "lastTextIndex";
			const lastIndex = useParentIndexing ? parentIndexMap.get(parentToolUseId) ?? -1 : state[globalIndexKey];
			const otherIndex = useParentIndexing ? otherParentIndexMap.get(parentToolUseId) ?? -1 : state[otherGlobalIndexKey];
			if (lastIndex >= 0 && lastIndex > otherIndex && state.contentBlocks[lastIndex]?.type === type) {
				...
				if (useParentIndexing) parentIndexMap.set(parentToolUseId, newIndex);
				else state[globalIndexKey] = newIndex;
			}
		}
		/**
		* 处理 tool_call
		...
		*/
		mergeToolCallWithConverter(tool, update, notification, state) {
			...
			if (acpStatus !== void 0) {
				...
				if (isTerminalACPToolStatus(acpStatus) && state && tool.name?.toLowerCase() === "task" && tool.id) {
					state.parentReasoningIndex.delete(tool.id);
					state.parentTextIndex.delete(tool.id);
				}
			}
			...
			return {
				message: {
					...
					extra: {
						...
						...state.lfConvId ? { lfConvId: state.lfConvId } : {},
						...state.lfConvReqId ? { lfConvReqId: state.lfConvReqId } : {},
		...
		*/
		...
		/**
		* 处理 suggest_plugin_install 的 session question 请求。
		* 同一 requestId 的 owner update 会替换 args，使连接中/失败等状态可跨切会话恢复。
		*/
		handlePluginRecommendationRequest(sessionId, toolCallId, args) {
			const mockNotification = {
				sessionId,
				update: { sessionUpdate: "tool_call" }
			};
		...
		*/
		...
		cleanupRemovedToolCall(state, toolCallId) {
			...
			state.parentTextIndex.delete(toolCallId);
		}
		...
		markPendingToolsAsFailed(sessionId, reason) {
			...
			for (let idx = 0; idx < state.contentBlocks.length; idx++) {
				...
				if (reason.preserveInteractiveQuestions) {
					if (isInteractiveQuestionTool(tool.name)) continue;
				}
...
//#endregion
export { init_acp_message_accumulator as n, ACPMessageAccumulator as t };
