...
var FileAuthenticationStorage = class FileAuthenticationStorage {
	...
	constructor() {
		this.storeSessionSubject = new require_common$1.Subject();
		this.watcher = null;
		this.watcherRetryTimer = null;
		this.watcherStableTimer = null;
		this.watcherRetryDelayMs = _FileAuthenticationStorage.WATCHER_RETRY_INITIAL_DELAY_MS;
	...
	static {
		this.IS_WRITING_HOLD_MS = 300;
	}
	static {
		this.UNLINK_CONFIRM_DELAY_MS = 500;
	}
	static {
		this.WATCHER_RETRY_INITIAL_DELAY_MS = 250;
	}
	static {
		this.WATCHER_RETRY_MAX_DELAY_MS = 3e4;
	}
	static {
		this.WATCHER_STABLE_RESET_MS = 3e4;
	}
	init() {
	...
	async store(session, options) {
		...
		try {
			this.beginWriting();
			committed = await require_credential_protection.withCredentialFileLock(filePath, () => {
				if (options?.preserveLogoutMarker && this.hasLogoutMarker(filePath)) return false;
				serializedSession = this.encodeSession(filePath, session);
				require_credential_protection.writeCredentialFileAtomicallySync(filePath, Buffer.from(serializedSession, "utf8"));
				if (options?.preserveLogoutMarker && this.hasLogoutMarker(filePath)) return false;
				if (!options?.preserveLogoutMarker) this.retireLogoutMarker(filePath);
				return !this.hasLogoutMarker(filePath) && this.ensureSerializedSession(filePath, serializedSession);
			});
			if (!committed || this.hasLogoutMarker(filePath)) {
			...
			this.createCredentialFailureReporter(filePath).record("write", error);
	...
	async restore() {
		...
		try {
			...
			const decoded = this.decodeSession(filePath, content);
			this.session = decoded.session;
			if (decoded.migrationNeeded) this.scheduleCredentialMigration(filePath);
			this.logger.info(`Restored auth session from file, userId: ${this.session?.account?.uid}`);
			this.emitSession(this.session);
			return this.session;
		} catch (error) {
			if (this.disposed) return;
			this.createCredentialFailureReporter(filePath).recordStorageFailure("read", error);
	...
	async beginLogout() {
		...
		try {
			this.beginWriting();
			await require_credential_protection.withCredentialFileLock(filePath, () => {
	...
	encodeSession(filePath, session) {
		const attempt = this.createCredentialFailureReporter(filePath).attempt("write");
		const codec = this.createAuthCodec(filePath, attempt.onUnavailable);
		try {
			if (fs.existsSync(filePath)) try {
				codec.decode(JSON.parse(fs.readFileSync(filePath, "utf8")), require_credential_protection.AUTH_CREDENTIAL_FIELDS);
			} catch {}
			const persisted = codec.encode(session, require_credential_protection.AUTH_CREDENTIAL_FIELDS);
			attempt.complete();
	...
	decodeSession(filePath, serialized) {
		const attempt = this.createCredentialFailureReporter(filePath).attempt("read");
		const codec = this.createAuthCodec(filePath, attempt.onUnavailable);
		try {
			const decoded = codec.decode(JSON.parse(serialized), require_credential_protection.AUTH_CREDENTIAL_FIELDS);
			const result = {
				session: decoded.value,
				migrationNeeded: decoded.migrationNeeded
			};
			attempt.complete();
			return result;
		} finally {
	...
	scheduleCredentialMigration(filePath) {
		require_credential_protection.scheduleCredentialFileMigration(filePath, (currentBytes) => {
			const attempt = this.createCredentialFailureReporter(filePath).attempt("migrate");
			const codec = this.createAuthCodec(filePath, attempt.onUnavailable);
			try {
				const decoded = codec.decode(JSON.parse(currentBytes.toString("utf8")), require_credential_protection.AUTH_CREDENTIAL_FIELDS);
				if (!decoded.migrationNeeded) {
					attempt.complete();
					return;
				}
				const persisted = codec.encode(decoded.value, require_credential_protection.AUTH_CREDENTIAL_FIELDS);
				const migrated = Buffer.from(JSON.stringify(persisted, null, 2), "utf8");
				attempt.complete();
				return migrated;
			} finally {
				codec.dispose();
			}
		}, { onFailure: (error) => this.createCredentialFailureReporter(filePath).record("migrate", error) });
	}
	createAuthCodec(filePath, onUnavailable) {
		return require_credential_protection.createCredentialFieldCodec({
			purpose: "workbuddy-auth-session",
			resourceId: `auth/${path.basename(filePath)}`,
			onUnavailable
		});
	}
	createCredentialFailureReporter(filePath) {
		return new require_credential_protection.CredentialProtectionFailureReporter({
			filePath,
			resourceId: `auth/${path.basename(filePath)}`,
