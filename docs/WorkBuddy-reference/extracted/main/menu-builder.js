...
function resolveStartupTypeSafe() {
	try {
		return (require_startup_type.init_startup_type(), require_chunk.__toCommonJS(require_startup_type.startup_type_exports)).getStartupType();
	} catch {
		return "cold";
	}
}
...
function aggregateStartupSession(logFilePath, startupTypeOverride) {
	...
	try {
		...
		const firstInteractiveMark = locatedMarks.find((m) => m.id === "E14");
		if (firstInteractiveMark) {
			const start = Math.min(...locatedMarks.map((m) => m.absEpoch));
			firstInteractiveMs = Math.round((firstInteractiveMark.absEpoch - start) * 100) / 100;
		}
...
*/
var startupMarkObserver = null;
/** 注册/注销启动打点观察者（传 null 注销）。仅 main 进程使用。 */
function setStartupMarkObserver(observer) {
	startupMarkObserver = observer;
}
/**
...
function markStartup(id, data) {
	...
	try {
		startupMarkObserver?.(id, data);
	} catch {}
}
/**
* 打一个「绝对时刻」启动点（只带 epochMs，行为对齐 markProcessCreated / markAbsolute）。
*
* 用途：调用点持有一个「过去时刻」的 epochMs（例如 index.ts 从 globalThis
* 拿到的「进程首行 JS 时刻」），需要以那个时刻为坐标落点，而不是「当下」。
*
* 与 `markStartup` 的关键差异：
* - 只写 epochMs，不写 performance.now 采样值 → 聚合器 `computeClockOffsets`
*   跳过它，不污染本进程 offset 中位数
* - 不写 elapsed → 段内 max-min 跳过它，不扭曲段时长
* - 不 push 进 `this.marks` → 本进程 summary 不参与
*
* 全程吞异常，绝不阻塞启动。
*/
function markStartupAbsolute(id, epochMs, data) {
	try {
		const def = require_package_and_show_log.STARTUP_MARKS[id];
		if (!def) return;
		require_logger$1.getWorkbuddyPerfLogger(require_logger$1.PerfFlow.STARTUP).markAbsolute(def.key, epochMs, {
			id: def.id,
			phase: def.phase,
			...data ?? {}
		});
...
function markDaemonObservedFromMain(id, data) {
	try {
		...
		if (!def || !ctx) return;
		require_logger$1.getWorkbuddyPerfLogger(require_logger$1.PerfFlow.STARTUP).appendRendererLine(JSON.stringify({
			_type: "mark",
			epochMs: Date.now(),
			phase: def.key,
			source: "daemon-rpc",
...
function markProcessCreated() {
	try {
		...
		const def = require_package_and_show_log.STARTUP_MARKS.A0;
		require_logger$1.getWorkbuddyPerfLogger(require_logger$1.PerfFlow.STARTUP).markAbsolute(def.key, creationMs, {
			id: def.id,
			phase: def.phase
		});
	} catch {}
...
function hasStartupFinalMark(rawJsonl) {
	if (!rawJsonl) return false;
	return /"id"\s*:\s*"F13"/.test(rawJsonl) || /"id"\s*:\s*"E14"/.test(rawJsonl) || /"id"\s*:\s*"B15"/.test(rawJsonl);
}
...
async function recordStartupPerfToCollector(payload) {
	try {
		...
		require_package_and_show_log.logStartupPerf(session.summary);
...
async function resolveCurrentStartupJsonlPath() {
	const ctx = require_startup_context.getStartupContext();
	const candidates = [];
	if (ctx) candidates.push(path.join(require_workbuddy_paths.getWorkbuddyLogsDir("startup"), ctx.date, `${ctx.pid}-${ctx.time}.jsonl`));
	const todayDir = ctx?.date ? path.join(require_workbuddy_paths.getWorkbuddyLogsDir("startup"), ctx.date) : path.join(require_workbuddy_paths.getWorkbuddyLogsDir("startup"), (/* @__PURE__ */ new Date()).toISOString().slice(0, 10));
	try {
...
/**
* 是否可打开启动报告窗口。
* 与其他性能诊断入口共享同一开关：只要开启了 `perfProfiling`
* （settings.json 或 WORKBUDDY_PERF_PROFILING env），就把 5 个诊断入口
* 一起显示。不判断登录、不区分 IOA / galileo。
*/
function canOpenStartupReportWindow() {
	return require_cdp_profiler.isPerfProfilingEnabled();
}
/**
* "复制启动 Trace ID" 命令处理：复制到剪贴板并弹提示。
* 抽出的目的：Windows/Linux 渲染层菜单通过 IPC 复用同一逻辑，避免两端行为漂移。
*/
function copyStartupTraceIdAction(locale) {
	...
	electron.dialog.showMessageBox({
		...
		message: t("traceIdDialogTitle"),
		detail: traceId ? t("traceIdCopiedDetail").replace("{traceId}", traceId) : t("traceIdNotAvailableDetail"),
		buttons: [t("ok")],
		noLink: true
	}).catch(() => void 0);
}
...
*/
function setMenuUserId(userId, _enterpriseId) {
...
async function executeMenuCommandFallback(commandId) {
	switch (commandId) {
		...
		case MENU_COMMAND_IDS.OPEN_LOGS_FOLDER: {
			...
			const primaryLogsDir = logFilePath ? path.dirname(logFilePath) : require_workbuddy_paths.getWorkbuddyLogsDir();
			await require_package_and_show_log.packageAndShowLog(primaryLogsDir, {
				openPath: (targetPath) => electron.shell.openPath(targetPath),
				showItemInFolder: (targetPath) => electron.shell.showItemInFolder(targetPath)
			}, [
				require_workbuddy_paths.getWorkbuddyLogsDir(),
				require_workbuddy_paths.getWorkbuddyPendingTelemetryDir(),
				require_logger.getElectronLogDir()
			].filter((d) => d !== primaryLogsDir));
			return;
		}
		...
		},
		{
			...
			submenu: [
				...isDocumentationDisabled() ? [] : [{
					...
					click: async () => {
						await electron.shell.openExternal("https://www.workbuddy.cn/docs/workbuddy/Overview");
					}
				}],
				{
					label: t("selfCheck"),
					click: () => executeMenuCommand(MENU_COMMAND_IDS.SELF_CHECK)
				},
				...
				...require_cdp_profiler.isPerfProfilingEnabled() ? [
					...
					{
						label: t("copyStartupTraceId"),
						click: () => copyStartupTraceIdAction(currentLocale)
					},
					{
						label: t("openStartupReport"),
						click: () => {
							openStartupReportWindow().catch(() => void 0);
						}
					}
				] : [],
				..._loggedInUserId ? [{ type: "separator" }, {
					label: t("copyUserId"),
					click: () => {
...
Object.defineProperty(exports, "copyStartupTraceIdAction", {
	enumerable: true,
	get: function() {
		return copyStartupTraceIdAction;
	}
});
...
Object.defineProperty(exports, "markStartupAbsolute", {
	enumerable: true,
	get: function() {
		return markStartupAbsolute;
	}
});
...
Object.defineProperty(exports, "setStartupMarkObserver", {
	enumerable: true,
	get: function() {
		return setStartupMarkObserver;
	}
});
