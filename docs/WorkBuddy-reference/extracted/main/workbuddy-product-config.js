...
*
*   // anywhere
*   resolveBundledAsset('plugins', 'workbuddy-builtin', 'skills'); // → first existing path
...
/**
* 解析 WorkBuddy 数据目录名（不含 home，如 `.workbuddy` / `.aimea`）。
*
* 专享版/私有化由 apply-brand-patch.js 写入 product.json 的 dataFolderName；
* 缺失时回退 `.workbuddy`。作为目录名解析的单一权威，供 resolveWorkbuddyConfigDir
* 与 app-instance.getDefaultConfigDirname 共用，避免 trim/兜底逻辑不同步。
*/
...
/**
* 解析 WorkBuddy 数据目录（专享版/私有化统一入口）。
*
* 优先级：
*   1. WORKBUDDY_CONFIG_DIR / CODEBUDDY_CONFIG_DIR env（configureElectronApp 后已设置）
*   2. cli/product.json 的 dataFolderName（专享版由 apply-brand-patch.js 写入，如 `.aimea`）
*   3. 回退 `.workbuddy`
*
* 所有需要落盘到"用户数据目录"的底层模块（vendor 解压日志、pulse、
* dev-env override 等）都应经此函数取目录，避免各自硬编码 `.workbuddy`
* 导致专享版下额外生成 `~/.workbuddy`（cnb.woa.com/genie/genie/-/issues/79541）。
*/
...
function resolveAccProductConfigCachePath() {
	return path.join(resolveWorkbuddyConfigDir(), "cache", "acc-product-config-v3.json");
}
...
function assignAccProductConfigV3ToProcessEnv(json) {
	if (Buffer.byteLength(json, "utf8") <= ACC_PRODUCT_CONFIG_INLINE_ENV_MAX_BYTES) {
...
var init_workbuddy_product_config = require_chunk.__esmMin((() => {
	...
	require_dev_env_override.setDevEnvStagingEndpointResolver({ resolve: () => {
		const product = tryGetWorkbuddyBaseProductConfiguration();
		return product?.isOversea === true ? product.stagingEndpoint : void 0;
	} });
}));
//#endregion
...
Object.defineProperty(exports, "getAssetsRoot", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "resolveWorkbuddyConfigDir", {
	enumerable: true,
	get: function() {
		return resolveWorkbuddyConfigDir;
	}
});
Object.defineProperty(exports, "resolveWorkbuddyDataFolderName", {
	enumerable: true,
	get: function() {
		return resolveWorkbuddyDataFolderName;
	}
});
