...
*/
function parseChatRecordZipInternal(input, extractEntriesToDisk) {
	...
	const extractedDirPath = extractEntriesToDisk ? createExtractionDir(filePath, extractionRootDir) : void 0;
	if (extractedDirPath) try {
	...
	const mediaFiles = entries.filter((e) => !e.isDirectory && isMediaEntry(e.entryName)).flatMap((e) => {
		...
		const mediaPath = extractedDirPath ? getExtractedEntryPath(extractedDirPath, e.entryName) : void 0;
		return [{
			name,
			size: e.header.size,
			mime: guessMime(name),
	...
	const transcriptPath = extractedDirPath ? getExtractedEntryPath(extractedDirPath, txtEntry.entryName) ?? node_path.join(extractedDirPath, transcriptFileName) : filePath;
	...
	return {
		...
		fileSize: getFileSize(filePath),
		metadata,
		extractionStatus: extractedDirPath ? "ready" : "pending",
		extractedDirPath: extractedDirPath ?? "",
		chatHistoryText,
		truncated
	};
}
function parseChatRecordZipMetadataOnly(input) {
	return parseChatRecordZipInternal(input, false);
}
function parseChatRecordZip(input) {
	return parseChatRecordZipInternal(input, true);
}
var extractChatRecordZip = parseChatRecordZip;
//#endregion
//#region ../../packages/workbuddy-server/src/wechat-chat-history/service.ts
var DEFAULT_ENSURE_TIMEOUT_MS = 6e4;
var COMPLETED_TASK_TTL_MS = 1800 * 1e3;
var extractionTasks = /* @__PURE__ */ new Map();
function taskSnapshot(task) {
	if (task.result) return task.result;
	if (task.status === "ready") return {
		chipId: task.chipId,
		status: "failed",
		error: "missing ready result"
	};
	return {
		chipId: task.chipId,
		status: task.status,
...
function scheduleCompletedTaskCleanup(task) {
	if (task.cleanupTimer) clearTimeout(task.cleanupTimer);
	const timer = setTimeout(() => {
		if (extractionTasks.get(task.chipId) === task) extractionTasks.delete(task.chipId);
...
function createWorkbuddyWechatChatHistoryService(deps = {}) {
	...
	const getOrCreateTask = (params) => {
		const existing = extractionTasks.get(params.chipId);
		...
		extractionTasks.set(params.chipId, task);
		return task;
	};
	const runExtraction = (task) => {
		if (task.promise) return task.promise;
		task.status = "extracting";
		logger?.info?.("[WechatChatHistory] extraction started", {
			chipId: task.chipId,
		...
		task.promise = new Promise((resolve) => {
			setImmediate(() => {
				try {
					const parsed = extractChatRecordZip({
						filePath: task.filePath,
						extractionRootDir: task.extractionRootDir
					});
					if (!parsed.valid || !parsed.extractedDirPath) {
						const result = {
							chipId: task.chipId,
							status: "failed",
							error: !parsed.valid ? parsed.reason : "missing-extracted-dir"
						};
						task.status = "failed";
						task.error = result.error;
						task.result = result;
						scheduleCompletedTaskCleanup(task);
						logger?.warn?.("[WechatChatHistory] extraction failed", {
							chipId: task.chipId,
							error: result.error
						});
						resolve(result);
					...
					const result = {
						chipId: task.chipId,
						status: "ready",
						extractedDirPath: parsed.extractedDirPath,
						mediaFiles: parsed.metadata.mediaFiles
					};
					task.status = "ready";
					task.result = result;
					scheduleCompletedTaskCleanup(task);
					logger?.info?.("[WechatChatHistory] extraction completed", {
						chipId: task.chipId,
					...
					task.result = result;
					scheduleCompletedTaskCleanup(task);
					logger?.error?.("[WechatChatHistory] extraction threw unexpectedly", error);
					resolve(result);
				}
			});
		});
	...
	return {
		async parseZipMetadata(params) {
			try {
				const result = parseChatRecordZipMetadataOnly({
					filePath: params.filePath,
					maxChatHistoryChars: params.maxChatHistoryChars,
					extractionRootDir: params.extractionRootDir
				});
				if (!result.valid) logger?.info?.("[WechatChatHistory] parseZipMetadata rejected", {
					filePath: params.filePath,
					reason: result.reason
				});
				else logger?.info?.("[WechatChatHistory] parseZipMetadata ok", {
					filePath: params.filePath,
				...
				logger?.error?.("[WechatChatHistory] parseZipMetadata threw unexpectedly", error);
				return {
					valid: false,
		...
		async startExtraction(params) {
			const task = getOrCreateTask(params);
			if (task.status === "ready" || task.status === "failed") return taskSnapshot(task);
			runExtraction(task).catch(() => void 0);
			return taskSnapshot(task);
		},
		async getExtractionStatus(params) {
			const task = extractionTasks.get(params.chipId);
			if (!task) return {
				chipId: params.chipId,
				status: "pending"
			};
		...
		async ensureExtracted(params) {
			const task = getOrCreateTask(params);
			if (task.status === "ready" || task.status === "failed") return taskSnapshot(task);
			const extraction = runExtraction(task);
			const timeoutMs = params.timeoutMs ?? DEFAULT_ENSURE_TIMEOUT_MS;
			let timeout;
			const timeoutResult = new Promise((resolve) => {
				timeout = setTimeout(() => {
					logger?.warn?.("[WechatChatHistory] ensure extraction timeout", {
						chipId: params.chipId,
						timeoutMs
					});
					resolve({
			...
			const result = await Promise.race([extraction, timeoutResult]);
