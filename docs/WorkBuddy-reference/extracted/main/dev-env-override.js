...
*/
/**
* 注入当前品牌 product 配置中的 staging endpoint。
*
* 内置常量只作为产品配置尚不可用时的兜底，避免海外版 staging 被固定路由到国内预发。
*/
function setDevEnvStagingEndpointResolver(resolver) {
	devEnvStagingEndpointResolver = resolver;
}
/**
...
function resolveEndpointOverride() {
	...
	try {
		const productStagingEndpoint = devEnvStagingEndpointResolver.resolve()?.trim();
		if (productStagingEndpoint) return productStagingEndpoint;
	} catch {}
	return DEV_ENV_ENDPOINTS.staging;
}
...
var DEV_ENV_ENDPOINTS, devEnvStagingEndpointResolver, ARDOT_DEV_ENV_ENDPOINTS, devEnvOverrideForceEnabled;
var init_dev_env_override = require_chunk.__esmMin((() => {
	DEV_ENV_ENDPOINTS = {
		prod: "https://copilot.tencent.com",
		staging: "https://staging.codebuddy.cn"
	};
	devEnvStagingEndpointResolver = { resolve: () => void 0 };
...
Object.defineProperty(exports, "setDevEnvStagingEndpointResolver", {
	enumerable: true,
	get: function() {
		return setDevEnvStagingEndpointResolver;
	}
});
