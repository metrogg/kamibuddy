...
function emitSessionEvent(event: SessionEvent): void {
	...
	// 信封的 sessionId 取折叠后的当前会话 id：单宿主时代所有事件都属于它；
	// 会话注册表落地后改为按事件来源宿主打 id（Task 2.4），事件本体不动。
 ...
 *
 * 与审批的差别在关闭路径：会话作废（resetSession）时这里会显式 reject
 * 全部挂起 —— 宿主没了答案就永远送不到，挂着的 execute 必须响亮收尾，
 * 不能把永远等不到的 Promise 留在 Map 里。
 */
...
/** 会话作废时拒掉全部在途问卷（见 pendingQuestionnaires 注释）。 */
function rejectPendingQuestionnaires(reason: string): void {
	for (const [, slot] of pendingQuestionnaires) {
...
/**
 * 当前会话的 MCP 连接器 handle（建会话时随扩展一起组装）。
 * MCP 设置的三个 IPC handler 经它查运行态、触发热重载；
 * 会话作废（resetSession）即清空 —— teardown 后的 handle 是死的，
 * 状态是陈旧的，不该再被 handler 摸到。
 */
let mcpClientHandle: McpClientHandle | undefined;
...
async function createHost(sessionManager?: SessionManager): Promise<SessionHost> {
	 ...
	 * handle 暴露运行态查询与 reload：MCP 设置的 IPC handler 经它
	 * 热应用配置变更，不必重启会话。建会话成功后才挂到模块级
	 * （SessionHost.create 失败时扩展从未加载，handle 是空的）。
	 */
	...
	const host = await SessionHost.create({
		...
		extensions: [
			...
			questionnaireExtensionFactory({ requestAnswers: requestQuestionnaireAnswers }),
			...
			createDocReadTool(),
			// MCP 连接器（handle 说明见上方 mcpClient 的注释）。
			mcpClient.extension,
...
async function resetSession(): Promise<void> {
	...
	// 会话没了，在途问卷的答案永远送不到 —— 拒掉让挂着的 execute 响亮收尾。
	// 正常路径走不到这（流式中禁止换空间/新建任务），这是异常时序的兜底。
	rejectPendingQuestionnaires("会话已结束，这次提问不会有回答了");
	// dispose 已触发扩展 teardown（连接断开），handle 里的状态随之失效。
	mcpClientHandle = undefined;
	/*
	...
	return infos
		.map((info): SessionSummary => {
			...
			return {
				...
				current: isCurrent,
				// 单宿主时代只有当前会话可能在跑；会话注册表落地后改为按会话查
				// 各宿主的运行态（Task 2.4），这里不预先造结构。
				running: isCurrent && conversation.state.isStreaming,
			};
		})
...
const handlers: Record<string, Handler> = {
	...
	[INVOKE.mcpConfigGet]: async (): Promise<McpConfigSnapshot> => {
		...
		try {
			const config = readMcpConfig(workspaceDir);
			return {
	...
	[INVOKE.mcpConfigSet]: async ([configJson]) => {
		...
		// 热应用：无会话时为空操作，下个会话自然读到新配置。
		await mcpClientHandle?.reload();
	},
	...
	// 单根时代只认当前工作区（cwd 比较的 === 口径与 applyWorkspace 一致）；
	// PreviewServer 多根落地后改为按 cwd 查实例表（Task 2.7）。
	[INVOKE.previewBaseUrl]: async ([cwd]) =>
		cwd === workspaceDir ? previewServer.baseUrl : undefined,
