 ...
 * 不依赖模型回想工具描述里的某一行。
 *
 * 2026-09-19 补第二种成因（§4.31）。原先文案只认「目标在工作目录外的文件」，
 * 结尾还写着「若目标本应在工作目录内，请检查路径」—— 而真机里出现过完全相反的情形：
 * 目标**就在区内、路径也没错**，写不进去是因为那个**目录**是程序自己建的：
 * Python 的 `tempfile.mkdtemp()` / `mkdir(0o700)` 会用一个 mode 派生、**不继承父目录**的
 * DACL 建目录，我们授给沙箱的能力 ACE 不在里面，于是 Errno 13。实测（ppt-master 的
 * `finalize_svg.py`）里模型正是卡在这里：它按旧文案怀疑自己的路径写错了，转半圈才
 * 自己推断出另一种可能。断言这句会误导人 —— 所以文案改成给两种成因，并点明
 * 第二种「别去改路径」。
 */
const DENIAL_MARKER =
	"提示：上面的失败看起来是**沙箱写约束**拒绝了写入（不是命令语法问题）——" +
	"当前档位只允许写工作目录内的文件。它有两种常见成因：" +
	"① 目标在工作目录之外；" +
	"② **目标在区内、路径也没错**，但那个目录是程序自己用私有权限建的：" +
	"Python 的 tempfile.mkdtemp() / mkdir(0o700) 会用一个不继承父目录的 DACL 建目录，" +
	"所以里面不含本沙箱的授权。第 ② 种改路径解决不了，别去改路径。";
 ...
 *
 * 适用条件原先只写「需要写到工作目录之外」，把上面第 ② 种（区内目录不归沙箱授权）
 * 排除在外了 —— 而真机里那种情形**只能**靠提权解，模型却因为文案的措辞不敢用
 * （它的原话：「错误提示说若确实需要写到工作目录之外才用。这里是工作目录内」）。
 * 现在两种都明说（§4.31）。
 */
const ESCALATION_HINT =
	"出路：带 sandbox_permissions + justification 重试**同一条命令**一次，由用户决定是否批准 ——" +
	"「要写到工作目录之外」与「区内目录不归沙箱授权」（上面第 ② 种）都适用，取最窄的够用档位。";
