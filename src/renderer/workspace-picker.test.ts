 ...
 * 要防的回归：每任务独立目录落地后，未选工作空间的新任务 cwd 变成
...
describe("pickerLabel（chip 显示的工作空间名）", () => {
	it("未选 / 待分配（空串、undefined）显示「临时任务」", () => {
		expect(pickerLabel("")).toBe("临时任务");
		expect(pickerLabel(undefined)).toBe("临时任务");
	});

	it("自动分配目录（时间戳）语义上仍是未选工作空间，显示「临时任务」", () => {
