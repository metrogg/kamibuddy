...
describe("team_member_progress 成员实时投影", () => {
	const member = (name: string, overrides: Partial<SubagentStatus> = {}): SubagentStatus => ({
		kind: "team",
		agent: name,
		task: `${name} 的任务`,
		status: "running",
	...
	it("投影归位到 team_create 卡（整体替换语义，无需 toolCallId）", () => {
		const view = apply([
			{ type: "tool_finished", card: toolCard({ toolName: "team_create", label: "建团队", outcome: "ok" }) },
			{ type: "team_member_progress", members: [member("sc1", { status: "done", turns: 4 })] },
		]);
		expect(view.entries).toHaveLength(1);
		expect((view.entries[0] as ToolCard).subagents).toEqual([member("sc1", { status: "done", turns: 4 })]);
	});

	it("多张 team 卡时归位到最近的一张（后建的团覆盖新的状态）", () => {
		const view = apply([
			{ type: "tool_finished", card: toolCard({ toolName: "team_create", label: "建团队", outcome: "ok" }) },
			{ type: "tool_finished", card: toolCard({ id: "t2", toolName: "team_create", label: "建团队", outcome: "ok" }) },
			{ type: "team_member_progress", members: [member("sc2")] },
		]);
		expect((view.entries[0] as ToolCard).subagents).toBeUndefined();
		expect((view.entries[1] as ToolCard).subagents).toEqual([member("sc2")]);
	});

	it("没有 team 卡（旧格式会话）→ 事件丢弃，不编造卡片", () => {
		const view = apply([{ type: "team_member_progress", members: [member("sc1")] }]);
		expect(view.entries).toHaveLength(0);
	});

	it("task 卡不算 team 卡（toolName 精确匹配，成员投影不污染子代理卡）", () => {
		const view = apply([
			{ type: "tool_started", card: toolCard({ toolName: "task", label: "委派子代理" }) },
			{ type: "team_member_progress", members: [member("sc1")] },
		]);
		expect((view.entries[0] as ToolCard).subagents).toBeUndefined();
	});
});
