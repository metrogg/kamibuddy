 ...
 *
 * ── 模型体验契约（三段；改行为必须同步改这里）──
 * What the model sees: hidden context 快照的 `<python_env>` 段里出现「托管运行时清单」
 * （逐项：id / 版本 / 状态 / 用途，就绪时附目录与解释器绝对路径；禁用项附「已被用户禁用，
 * 请用户去设置页开启」，未安装项附「尚未安装，请用户到设置页点安装」，失败项附「安装失败，
 * 请用户看诊断或重试安装」——三者各给唯一一条下一步）。这一段是**环境块**快照
 * （`kamibuddy-hidden-context`，三条快照通道之一）的一部分：每个 run 由 session-host 冻结、
 * 经 before_agent_start 落进会话文件（落在本轮用户消息之后），与环境块末条同 `customType`
 * 快照逐字节相同时不追加 —— 时间走独立的 `kamibuddy-run-time` 通道（spec:
 * add-supersede-note-and-time-split 起），本段不再因分钟一变跟着整块重发。
 * Token effect: 这一段只在**内容变化**、新追加一条快照时才付一次全价（常量级：条目数固定、
 * 每项正文两三行，约 120~200 tokens）；追加后它成为历史的普通一员，后续每轮按前缀命中价
 * 参与，不重复全价 —— 上一轮的快照副本**就在**下一轮请求里（位置固定，命中的就是它）。
 * KV Cache effect: 落点在**本轮用户消息之后**的一条持久快照条目（shared/hidden-context.ts
 * 的 shouldAppendSnapshot），且不改写任何既有条目（append-only）；运行时开关或路径变化只
 * 让快照内容变、追加一条新的，既有前缀不受影响。反例（2026-09-18 实测，spec:
 * persist-context-snapshots）：早先「每请求现算、不落盘」的尾部注入会让 `cacheRead_N`
 * 每轮被截断（gap 2,423…2,615，见 docs/可观测性清单.md CACHE8）。
 */
