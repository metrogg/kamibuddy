 ...
 * 会话导出 HTML 的路径构造。
 *
 * 输出目录固定为 `<生效根>/exports/`（生效根 = `getEffectiveWorkspaceRoot()`，
 * 由调用方 daemon 传入 exportsDir，本文件只拼文件名），而非当前会话 cwd：
 * 各任务的 cwd 不同，导出物落在其下会散落到各处；用户也总能在一个固定地方
 * 找到自己的导出物，不用记「当时用的哪个工作区」。
 * 用生效根而非内置默认根（`getWorkspaceDir()`）：用户在设置里改「默认存储路径」
 * 改的就是生效根，导出物应随之落到新根 —— 这与「设置项优先」是同一条口径。
 *
