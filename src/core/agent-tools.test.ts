...
import { ensureAgentTools, type AgentToolsResult } from "./agent-tools.ts";
...
describe("ensureAgentTools", () => {
	...
	it("第二次：已就位就不动（幂等，installed 为空）", async () => {
		await ensure();
		const second = await ensure();
		expect(second).toMatchObject({ installed: [], present: ["fd", "rg"], missing: [] });
	});

	it("**不覆盖**已有文件：用户/环境里放的版本优先", async () => {
	...
	it("随包资产缺某个二进制：算 missing，不编造也不阻断另一个", async () => {
	...
	it("非 Windows：明确跳过并给出原因（不会去拷 .exe）", async () => {
		const result = await ensureAgentTools({ sourceDir, targetDir, platform: "darwin" });
		expect(result.skipped).toContain("Windows");
		expect(result.installed).toEqual([]);
		expect(existsSync(targetDir)).toBe(false);
	});

	it("什么都没得装时也不建空目录", async () => {
		rmSync(join(sourceDir, "fd.exe"));
	...
	it("默认落点是 pi 自己的 bin 目录（`getAgentDir()/bin`，不是我们的配置目录）", async () => {
		/*
		 ...
		 */
		const result = await ensureAgentTools({ sourceDir, binaries: [] });
