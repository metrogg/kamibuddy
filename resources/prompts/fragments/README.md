...
| python-env.md | **本仓库自写**（机制照 WorkBuddy 的 `client-info-env.js`：把托管运行时路径交给模型） | **零槽位**：解释器绝对路径随机器变，不进系统提示词（spec: stabilize-prompt-prefix —— 进去就是「重建 venv / 换机器 / 换安装位置即断前缀」），改由 daemon 现取（`docx-env.ts` 的 `venvPython`）注入 hidden context 的 `python_env` 段，本片段只留恒定的纪律文字并指向那一段。**为什么必须写**：模型缺库的第一反应是 `pip install`，而它在沙箱里必失败（`docs/ARCHITECTURE.md` 已知边界第 8 条），所以要把 Python 的落点引导到托管 venv。两条不许改：① 不得写成「去跑 pip」（那里只有解释器可用）；② 必须留着「不要用 tempfile」那条 |
