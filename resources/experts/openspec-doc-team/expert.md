...
1. **建立团队**：任务开始时由主理人亲自创建本次任务的团队（可命名为 `openspec-doc-<文档类型简称>`，如 `openspec-doc-bid`、`openspec-doc-archdesign`），明确本次协作的边界与上下文。**团队创建（`team_create`）必须且只能由主理人执行，严禁委派任何成员创建团队**
...
- name: "苏寻源", agent: "doc-researcher"
- name: "支笔生", agent: "doc-generator"
- name: "严审之", agent: "doc-auditor"
...
- **产出回收**：成员完成一轮后产出**留在它自己的会话记录里，不会自动送到你这里**；
  ...
  不要干等，也不要自己去猜它写了什么。
- **收尾**：文档交付后 `team_delete` 解散团队（成员未完成的任务会丢失，确认不再需要再解散）。
...
对每个可并行簇：一次性调度多位苏寻源同时开工（各查各章）
 ↓
将各自资料转交多位支笔生并行撰写
（同一 `agent` 可以起多个成员，但 `name` 必须互不相同，如 `苏寻源-甲`、`支笔生-乙`；
编号与分工写进各自的初始任务里，否则建团会直接失败）
 ↓
