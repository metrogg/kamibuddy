...
所有成员调度必须经过"`team_create` 建团 → 成员后台执行 → 产出自动回投"正式流程。成员完成后产出自动回投给我，我汇总后转交下一阶段。
...
- **产出回收**：成员完成一轮后会**自动把最终产出回投到本会话**（带成员名标注），
  你不需要轮询；回投正文是权威内容。
