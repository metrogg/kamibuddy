...
description: 设计或改造网页/界面时的视觉设计指导——先定主题与受众、出设计计划（色板/字体/布局/原则）、规避模板化「AI 味」、写码后截图自审。当用户要求做网页、落地页、仪表盘、HTML 报告或界面美化时使用。
---

<!-- 来源：anthropics/claude-code plugins/frontend-design（MIT License，见同目录 LICENSE.txt）。
     正文保留英文原文以便与上游比对；description 中文化贴合我们的触发场景。 -->
