...
| 字段               | 类型     | 必填 | 说明                                                                                                |
| ------------------ | -------- | ---- | --------------------------------------------------------------------------------------------------- |
| `request_id`       | string   | ✅   | 唯一请求 ID（任务名 slug），同一次请求所有 Stage 共享                                                |
| `entry`            | enum     | ✅   | 入口类型（本流程承接）：full_pipeline（编辑类意图不进本流程、不写本 log，直接就地处理）              |
...
| `target_skill`     | string   | ✅   | 路由到的子角色 / 能力名称                                                                           |
...
本编排层承接的入口仅 `full_pipeline`，Stage 链见 `SKILL.md §Stage 0 → 常见预设`（唯一真相源），此处不重复。

> 编辑类意图（在既有文档上改动/润色/重排等）**不进本流程、不写本 log**——识别到即直接在当前会话用 read/edit 就地处理。

## Route 标签

`route` 字段由**各 Stage 的执行子角色** 填写，其取值由对应子角色定义（见 `agents/doc-writer.md` / `agents/doc-formatter.md` / `agents/doc-converter.md`）。
本文件不枚举子角色的内部 route 名，避免与子角色漂移。
