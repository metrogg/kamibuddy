...
```yaml
schema_version: 1
request_id: <任务名>            # 本次任务的简短主题 slug，同时是 output/ 下的目录名
created_at: <ISO-8601>
updated_at: <ISO-8601>

entry_type: full_pipeline
                          # 本编排层承接的入口仅 full_pipeline（以 S3 doc-converter 转换收尾）
                          # ⚠️ 编辑类意图（含润色/重排）不进本流程、不写本文件——
                          #    识别到即直接在当前会话用 read/edit 就地处理
stage_chain: [1, 2, 3]    # 由 entry_type 映射，映射表见 SKILL.md §Stage 0 → 常见预设（唯一真相源）

# 最终 .docx 交付路径（Stage 0 按路径约定解析为绝对路径并写入）
...
stages:
  ...
  stage_2:
    ...
    started_at: null | <ISO-8601>
    completed_at: null | <ISO-8601>
    route: null | html                                    # 排版恒为 html（doc-formatter 只产 html）
    html_sub: null | template
  ...
  stage_3:                                              # 本编排层内 S3 = doc-converter（HTML→DOCX 转换 + present_files）
    ...
    skill_used: []                                        # 有序数组：本次 S3 使用的转换通道；本编排层内恒为 ["docx_convert"]
    ...
    # ⚠️ 编辑（改动/润色/重排）不在本流程内承接：
    #    识别到即直接在当前会话用 read/edit 就地处理，
    #    不创建 request_id / pipeline-state.yaml，不在本轮内累积。
...
| 节点                   | 动作                                                                                                                                                                                                              |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Stage 0 判定入口后** | 创建 `output/<任务名>/` 目录结构；按「输出路径规范」决定 `output_docx_path`；写入 `schema_version / request_id / created_at / entry_type / stage_chain / current_stage / output_docx_path / output_docx_user_specified`；`stage_chain` 内各 stage 设 `pending`，不在链中的设 `skipped`                   |
...
| **Stage 3 完成后**     | 本编排层内 S3 = **doc-converter**（HTML→DOCX 转换）：完成时写入 `stage_3.executor="doc-converter"` / `skill_used=["docx_convert"]` / `output_path` 落盘 / `present_files_opened=true`（doc-converter 内部硬约束），`status` 置为 `completed`，本流程结束 |
...
| ----------------------------- | ------------------- | --------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------- |
| 转换 + 预览（S3）             | `"doc-converter"`   | `["docx_convert"]`    | 拿到 .docx **必须**立即 `present_files` 打开预览 → 记 `present_files_opened=true` | doc-converter 完成即 `completed`，本流程结束   |
...
Pipeline 中间产物落在 `output/<任务名>/` 下；最终 .docx 交付路径由顶层 `output_docx_path` 决定。
...
├── stage2/
│   ├── design_tokens.json           # design-token 输出
│   ├── formatted-<主题>.html        # 排版产物
...
Stage 0 按以下约定决定 `output_docx_path` / `output_docx_user_specified` 并写入 yaml；doc-converter 原样使用。

- 用户指令显式给出路径 → 用该路径（绝对路径），`output_docx_user_specified: true`
- 用户未指定 → `<工作区>/output/<任务名>/stage3/<主题>.docx`（文件名按主题决定），`output_docx_user_specified: false`

`stage_3.output_path` 与顶层 `output_docx_path` 同值。父目录用 write 工具创建（写入第一个文件时自动建立）。
...
3. **`stage3` 的 .docx 路径由 `output_docx_path` 决定**（Stage 0 按路径约定确定）。
