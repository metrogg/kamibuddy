...
> 落点：基类 `.entry-toolbar` 只留行内布局，显隐分叉到 `.entry-toolbar-right`（用户侧，
> `opacity: 0` → hover/focus-within 时 1）与 `.entry-toolbar-left`（助手侧，常驻且占布局
> 高度 —— 用户侧那套浮条上缘会压掉本条最后一行文本，hover 一闪不显眼，常显就是遮挡）。
> ② **不再是「只含一个复制按钮」**：现为「执行计划（仅 plan 模式末条）→ 复制 → 重试
...
> **【2026-09-15 订正】** 上面两条 Scenario 的显隐口径只对**用户侧气泡**成立。助手侧操作条
> 常驻（未 hover 也可见，故没有「复制按钮要点 hover 才点得到」这一步）；它同样占布局高度，
> 所以「不打扰阅读」的实质要求（消息间距不随 hover 跳动）依然满足。
