...
# 第五轮：页脚 token 超计根因 + 探针归因定案（2026-09-17）

**验证修复 1 的真实会话证据**（1 轮 1 步的干净实验）：轮 2 的 prompt 只比轮 1 多 0.3K，而它装着轮 1 的全部内容
（按 `|prompt2|−|prompt1| = 输出 249 + 新用户 + 新注入 ≈ 0.26K` 复算，分毫不差）⇒ 轮 1 整段由缓存服务。
修复前同位置是 21.5% / 65.6%（整段重付）。页脚/面板/底栏三处读数也互相自洽（12.6K = 12.6K，24.8K = 12.3K+12.6K）。

- [x] 页脚 token 超计：**根因已定位并修复**（不是公式错）
  - 根因：消息 id 跨**宿主代际**复用（`idSeq` 进程内自增、`remountHostInBucket` 后从 1 重来），
    而 `assistant_done` / 流式增量走 `replaceEntry` 的 `findIndex`（**第一条**）
    ⇒ **新一代的正文与 usage 被盖进上一代那条老条目**，usage 跨轮搬家
  - 真实会话实测（6 轮 / 50 步）：轮 2 多算 **+132%**、轮 3 **整轮丢成 0**、轮 4 少算 179K；修复后 **6/6 与台账按 run 聚合逐位相等**
  - 修法：`replaceEntry` 加 `match:"first"|"last"`（assistant 三处更新用 `last`；工具卡保持 `first`，id 由 provider 生成天然唯一）；
    并把「最后一条 user 消息」边界收拢成 `shared/conversation.ts` 的唯一实现（页脚/轮切分/chat-view/reducer 全读它）
  - 门禁：合成「两代宿主 id 撞名 + 3 步多步轮 + 2 次工具调用」用例；改坏 → 3 例红（`expected 96429 to be 27038` 等）；还原全绿
  - **`↑12.1M` 那一例未能复现**：该会话真实事件流重放后最后一轮 = 2,130,693（与台账逐位相等）。
    可疑巧合：那一刻「全进程累计 billed ≈ 12.2M」（Σ 全部台账 = 12,221,547）与 12.1M 同量级 ⇒ 更像**同屏读数读串**。待用户确认
- [x] 探针两处修完
  - 截断根因：**不在脚本**（逐码点扫描无控制字符、打印路径无 slice）——是下游终端/采集对长行的**中段省略**。
    加固：退出前显式等 stdout 写回调；关键读数（hit / cacheRead）各占一行短行
  - **system vs tools 差分定案**：只改 system → 首步 cacheRead **0（0.0%）**；只改 tools（system 字节冻结）→ **5248（91.5%）**
    ⇒ 命中段在 **system / 消息一侧**，tools 不在命中段起始位置。算术旁证：7 个 tools ≈ 1272 估算 tokens，与 5888 量级不符
  - 「上一版 0.0% vs 这版 5120」**已解释**：provider 前缀缓存是**账号级**的，同一串字节发过一次下次就命中
    ⇒ **复跑会把「变更轮」也变成已缓存**（实证：同内容两发，A 轮 1 27.2%→96.3%，B 轮 2 变更轮 27.1%→95.8%）。修法：被改动的轮加逐 run nonce

**dsh 对照（关键收获）**：dsh 的 `tokenUsage` 是「**同一次 attempt 的最终 assistant 样本替换流式样本**，作用域由生命周期事件
（`llm/retry-started`）划定」——**我们的 reducer 就是这个 fold，而本次缺陷正是「折叠出的条目不再是 attempt 级事实」**。
他们另一条纪律我们也该学：**「证据不齐就不出数」**（`deriveTurnTokenUsage` 在证据缺失/计数不安全/总数冲突时返回无结果），
而我们页脚是「猜边界 + 照出数」。

**合并态验收**：`npm run check` exit 0（typecheck + check:deps + check:tokens）；`npm test` **126 文件全绿**。

**仍未修（同属「让条目带身份」，建议合成一个单独改动）**：
1. id 撞名的**根治**：给 `idSeq` 加代际命名空间（现在还会让 React `key` 重复 —— `chat-view` 用 `entry.id` 作 key）
2. **compaction 的模型调用不写台账**（`settleLlmCall` 在无 `turn_start` 时跳过）→ 若它发 `assistant_done`，页脚会计入而台账/会话卡不计
3. **steer/followUp 的页脚边界**（按「最后一条 user」猜边界的固有限制；根治同样是条目带 run 身份）
