...
- [x] **默认不影响生产**（默认不执行）：仅 `localStorage.kbPerf === "1"` 或 DEV 下且 `kbPerf !== "0"` 才执行
      ...
      **〔2026-09-15 订正〕** 首行「仅 `"1"` 或 DEV 下且 `kbPerf !== "0"`」的默认策略已作废：
      现为**只有 `"1"` 才执行**，未设置 / `"0"` 都不执行（**DEV 下亦然**）。改因：浮层是**主动取证**
      才需要的工具（改渲染性能时才量一次），DEV 下默认常驻会遮挡右下角界面、打扰正常开发。
      守卫现为 `if (perfFlag === "1")`（已无 `import.meta.env.DEV` 分支）；**上面"生产仍打包该 4.9 kB
      chunk"的结论不变**——守卫读的仍是运行时 localStorage，rollup 依旧判不了该死分支。
