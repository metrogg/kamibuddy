...
- [ ] Task 6: MetaFold 过程折叠
  - [ ] 6.1 shared 层纯函数：按 user 消息分组成「回合」，已完成回合内连续 tool 条目归为一个折叠单元；摘要生成（动作词映射 read→读取/write→写入/edit→修改/bash→运行命令…，归类计数「读取 3 个文件、写入 2 个文件」，兜底「N 个工具调用」）
  - [ ] 6.2 补 vitest：分组、摘要、进行中不折叠、混合条目边界
  - [ ] 6.3 chat-view 渲染折叠行（摘要 + chevron，点击展开/收起，按回合记住展开状态）
- [ ] Task 7: 用户消息气泡 + hover 工具条
  - [ ] 7.1 气泡样式（右对齐、圆角 16/16/0/16、fit-content、max-width、超高滚动）
  - [ ] 7.2 hover 工具条：智能时间戳（当天/昨天/当年/跨年四档，纯函数 + 单测）+ 复制（对勾 2s）
- [ ] Task 8: 回到底部 + 滚动跟随
  - [ ] 8.1 wheel 上滚停跟随、回底恢复；跟随逻辑替换现有「无条件贴底」useEffect
  - [ ] 8.2 「回到底部」圆形按钮（底部居中、阴影、入场动画）
- [ ] Task 9: 错误卡与重试
  - [ ] 9.1 run_error 渲染为内嵌错误卡（图标 + message + runId 复制 + 结构化报告复制）
  - [ ] 9.2 重试按钮：重发最后一条 user 消息（无 user 消息隐藏）
  - [ ] 9.3 替换现有 lastError 单行展示
